    ...
    <div style="min-height:25px;">
        <form action="users.php" method="get">
            ...
            <div class="attached input">
                <input type="text" class="basic-search" placeholder="Search Users" id="basic-user-search" name="query"
        ...
        var goBaby = function(action, confirmed) {
            ...
            if (ids.length) {
                var submit = function(data) {
                    ...
                    $.each(ids, function() {
                        $form.append($('<input type="hidden" name="ids[]">').val(this));
                    });
                    if (data)
                        $.each(data, function() {
                            $form.append($('<input type="hidden">').attr('name', this.name).val(this.value));
                        });
                ...
                var options = {};
                if (action === 'delete') {
                    options['deletetickets'] = __('Also delete all associated tickets and attachments');
